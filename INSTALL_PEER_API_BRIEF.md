...
| اشتراك مقيّد (`limit_control`/`inactive`) | 403 | `{"error":"Forbidden","message":"Subscription is currently restricted. Renew your plan to enroll new peers."}` |
| تجاوز حصة الأجهزة | 403 | `{"error":"Forbidden","message":"Peer limit reached (N/M). Upgrade plan to add more."}` |
...
- أجسام الأخطاء لم تعد تحمل حقول `subscription_status`/`current_peers`/`peer_limit`/`remaining_peers` — فقط `{error, message}`.
